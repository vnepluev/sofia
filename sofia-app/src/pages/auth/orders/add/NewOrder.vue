				...
				<q-card-section>
					<q-form class="p-4 space-y-4" @submit="nextStep">
						...
						<!--	Купон 000000-0000-0000
								...
								Пин код 0000 -->
						<q-input v-if="isCert" v-model="formData.sert_number" mask="XX####-####-####" hint="Номер купона"
							style="margin-top: -0.9rem;" />
						<q-input v-if="isCert" v-model="formData.sert_booking_code" mask="###" hint="Код бронирования"
							style="margin-top: -0.9rem;" />
						<q-input v-if="isCert" v-model="formData.sert_done_code" mask="####"
							hint="Код подтверждения (не обязательно)" style="margin-top: -0.9rem;" />

						<q-select v-if="!isCert" outlined v-model="formData.choiceYachtModel" :options="formData.choiceYacht"
						...
						<q-checkbox v-if="!isCert" class="text-right" left-label v-model="formData.isPromo"
							label="У меня есть промокод" checked-icon="task_alt" unchecked-icon="highlight_off" />

						<q-input style="margin-top: -1rem;" v-if="formData.isPromo" v-model="formData.promocode" type="text"
							hint="Промокод" />
			...
			<div class="w-72 q-card rounded-borders rounded-xl whitespace-pre-line">
				<q-card-section>
					<q-form class="p-4 space-y-4 font-bold" @submit.prevent="finalStep">
						<!-- form data -->
						<div class="text-sm" v-if="!isCert">{{ formData.choiceYachtModel }}</div>
						<div class="text-sm" v-if="isCert">Сертификат: {{ formData.sert_number }}</div>
						...
						<div class="text-sm" v-if="!isCert">Стоимость: {{ totalAmountSum }} руб.</div>
...
export default {
	props: {
		// если выбрали активация сертификата
		isCert: {
			type: Boolean,
			required: false,
			default: false
	...
	setup(props) {
		...
		const isForm1Valid = computed(() => {
			...
			let isValid = check1 === check2 && check1 === true;

			// если сертификат /V|N|\d/g
			if (props.isCert) {
				const check1Cert = formData.sert_number?.length === 16
				const check2Cert = formData.sert_booking_code?.length === 3
				const check3Cert = formData.sert_done_code?.length === 0 || formData.sert_done_code?.length === 4
				isValid = isValid && check1Cert && check2Cert && check3Cert
			}
			return isValid;
		});

		// проверка заполнения 2 формы
		const isForm2Valid = computed(() => (formData.people > 0) && isForm1Valid.value)
		...
		const finalStep = () => {
			...
			const finalData = {
				...
				photo: 0,
				water_circle: 0,
				sert_number: formData.sert_number,
				sert_booking_code: formData.sert_booking_code,
				sert_done_code: formData.sert_done_code
			};
